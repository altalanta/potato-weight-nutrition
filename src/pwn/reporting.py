        ...
        for model_type, results in model_results.items():
            if results.get("status") == "success":
                lines.extend([
                    ...
                    f"- R-squared: {results.get('r_squared', results.get('r_squared_within', 'N/A')):.4f}" if isinstance(results.get('r_squared', results.get('r_squared_within')), (int, float)) else f"- R-squared: N/A",
                    ""
                ])
                ...
                if "fiber_coefficient" in results:
                    lines.extend([
                        ...
                        f"**P-value**: {results.get('fiber_p_value', 'N/A'):.4f}" if isinstance(results.get('fiber_p_value'), (int, float)) else f"**P-value**: N/A",
                        ""
                    ])
